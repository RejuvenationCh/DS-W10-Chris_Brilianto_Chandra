...
4.3.e  Is shown on the main code
4.3.f is also shown on the main code
4.3.g is skipped
4.3.h It will only be the same if and only if the main tree only have one child or childless (Older generation will cry at this). Pre order will clean all the left side first then moves on onto the right side, BFS will clean all the side before going down a level. If the level is at most 1, BFS basically acts the same as Pre Order because it still scans from left to right.

4.3.I a. It will be O(n). Each directory is basically a node in the tree that require it to be accessed. For each directory and files that are scanned it becomes O(1)
